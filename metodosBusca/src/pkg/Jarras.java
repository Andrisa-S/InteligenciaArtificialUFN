...
public class Jarras implements Estado {
    ...
    final int TAM1 = 4; //capacidade jarra1
    final int TAM2 = 3; //capacidade jarra2
    ...
    public List<Estado> sucessores() {
        ...
        despejarJarra1_Jarra2(visitados);
        despejarJarra2_Jarra1(visitados);
    ...
    private void encherJarra2(List<Estado> visitados){
        Jarras novo = new Jarras(this.jarra1, this.TAM2, "Enchendo a jarra2");
        if (!visitados.contains(novo)){
            visitados.add(novo);
        }
    }
    ...
    private void esvaziarJarra1(List<Estado> visitados){
        Jarras novo = new Jarras(0, this.jarra2, "Enchendo a jarra1");
        if (!visitados.contains(novo)){
            visitados.add(novo);
        }
    }
    ...
    private void esvaziarJarra2(List<Estado> visitados){
        Jarras novo = new Jarras(this.jarra1, 0, "Enchendo a jarra2");
        if (!visitados.contains(novo)){
            visitados.add(novo);
        }
    }
    
    private void despejarJarra1_Jarra2(List<Estado> visitados) {
       int j1 = this.jarra1;
       int j2 = this.jarra2;
    ...
    private void despejarJarra2_Jarra1(List<Estado> visitados) {
